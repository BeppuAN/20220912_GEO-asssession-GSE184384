...
#load filtered gene matrix-derived dataset
...
#This step is computationally intensive make sure you free up some memory before performing this step
#performing this step on a super comuter or the cluster is highly recommended
epitimecourse.preintegration <- RunVelocity(object = epitimecourse.preintegration, deltaT = 1, kCells = 25, fit.quantile = 0.02)

#save progress up to this point
saveRDS(epitimecourse.preintegration, file = "epitimecourse.velocyto.V5.rds")

#attempt projection - this step is also computatioally intensive. 
#be sure to free up any space before atempting this step
ident.colors <- (scales::hue_pal())(n = length(x = levels(x = epitimecourse.integrated.V2)))
names(x = ident.colors) <- levels(x = epitimecourse.integrated.V2)
cell.colors <- ident.colors[Idents(object = epitimecourse.integrated.V2)]
names(x = cell.colors) <- colnames(x = epitimecourse.integrated.V2)
show.velocity.on.embedding.cor(emb = Embeddings(object = epitimecourse.integrated.V2, reduction = "umap"), vel = Tool(object = epitimecourse.preintegration, 
                                                                                                                                  slot = "RunVelocity"), n = 10, scale = "sqrt", cell.colors = ac(x = cell.colors, alpha = 0.5), 
